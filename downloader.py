...
b_download_subtitle = input("Download subtitles? (Y/n): ")
...
def dl_mp3(id, target_folder = './'):
    ...
    if not b_download_subtitle or b_download_subtitle.startswith('y') or b_download_subtitle.startswith('Y'):
        subtitle_url = re.findall(r"subtitle_url\":\"(.*?)\"", json_content)[0]
        if subtitle_url.startswith('http'):
            subtitles_stream = requests.get(subtitle_url)
            subtitle_fname = correct_fname(sound_title + '.json')
            with open(target_folder + '/' + subtitle_fname, 'wb') as f:
                f.write(subtitles_stream.content)
            # print(subtitle_fname + " downloaded in folder " + target_folder)
